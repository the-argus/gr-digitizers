...
public:
    ...
    gr::work::Status processBulk(gr::InputSpanLike auto& timingInSpan, std::span<TOutSpan>& outputs, gr::OutputSpanLike auto& digitalOutSpan) {
        ...
        const auto                            pollResult      = _picoscope->poll([&](std::span<std::span<const std::int16_t>> data, std::int16_t overflow) {
            ...
            for (const auto& [channelIdx, channelId, output] : std::views::zip(std::views::iota(0UZ), channel_ids.value, outputs)) {
                ...
                if (overflow & (1 << channelIdx)) {                                               // picoscope overrange
                    output.publishTag(gr::property_map{{"over-range", gr::pmt::Value{true}}}, 0); // todo: correct tag
                }
            }
        ...
        for (const auto& [channelIdx, output] : std::views::zip(std::views::iota(0UZ), outputs)) {
            ...
            for (auto& [index, map] : matchedTags.tags) {
                if (verbose_console && !chunkStartPublished && index > unpublishedSamples && nSamples > 0 && matchedTags.processedSamples > unpublishedSamples) {
                    output.publishTag(gr::property_map{{"chunk-start-time", gr::pmt::Value{std::chrono::duration_cast<std::chrono::nanoseconds>(acqStartTime.time_since_epoch()).count()}}}, unpublishedSamples);
            ...
            if (samplesDropped > 0UZ) {
                output.publishTag(gr::property_map{{"droppedSamples", gr::pmt::Value{samplesDropped}}}, unpublishedSamples + nSamples); // todo: correct tag
            }
        ...
        if (samplesDropped > 0UZ) {
            digitalOutSpan.publishTag(gr::property_map{{"droppedSamples", gr::pmt::Value{samplesDropped}}}, unpublishedSamples + nSamples); // todo: correct tag
        }
    ...
    [[nodiscard]] gr::property_map channelToTagMap(std::size_t channelIdx, float sampleRate) {
        ...
        return {
            {gr::tag::SIGNAL_NAME.shortKey(), gr::pmt::Value{getChannelSetting(std::span(signal_names.value), channelIdx, {})}},
            {gr::tag::SAMPLE_RATE.shortKey(), gr::pmt::Value{sampleRate}},
            {gr::tag::SIGNAL_QUANTITY.shortKey(), gr::pmt::Value{getChannelSetting(std::span(signal_quantities.value), channelIdx, {})}},
            {gr::tag::SIGNAL_UNIT.shortKey(), gr::pmt::Value{getChannelSetting(std::span(signal_units.value), channelIdx, {})}},
            {gr::tag::SIGNAL_MIN.shortKey(), gr::pmt::Value{offset - range}},
            {gr::tag::SIGNAL_MAX.shortKey(), gr::pmt::Value{offset + range}},
        };
    }
    ...
    requires(acquisitionMode == AcquisitionMode::RapidBlock)
    {
        ...
        ds.axis_names = {getChannelSetting(std::span(signal_names.value), channelIdx, {})};
        ds.axis_units = {getChannelSetting(std::span(signal_units.value), channelIdx, {})};

        ds.extents           = {static_cast<int32_t>(nSamples)};
        ds.layout            = gr::LayoutRight{};
        ds.signal_names      = {getChannelSetting(std::span(signal_names.value), channelIdx, {})};
        ds.signal_units      = {getChannelSetting(std::span(signal_units.value), channelIdx, {})};
        ds.signal_quantities = {getChannelSetting(std::span(signal_quantities.value), channelIdx, {})};
