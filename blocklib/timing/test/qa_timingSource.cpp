...
const suite TimingBlockHelpers = [] {
    ...
    "add metadata to tag"_test = []() {
        ...
        { // id does not match any filters -> HW-TRIGGER: false
            gr::Tag tag{.index = 0uz, .map = {{"existingKey", gr::pmt::Value{"test"}}}};
            gr::timing::TimingSource::addHwTriggerInfo(0x1136200000000000ul, tag, actionTrigger);
            gr::property_map expected{{"existingKey", gr::pmt::Value{"test"}}, {gr::tag::TRIGGER_META_INFO.shortKey(), gr::property_map{{"HW-TRIGGER", gr::pmt::Value{false}}}}};
        ...
        { // id matches first filter in the filter list -> HW-TRIGGER: true
            ...
            gr::property_map expected{{"existingKey", gr::pmt::Value{"test"}}, {gr::tag::TRIGGER_META_INFO.shortKey(), gr::property_map{{"HW-TRIGGER", gr::pmt::Value{true}}}}};
            expect(std::ranges::equal(expected, tag.map)) << [&tag, &expected]() { return std::format("got: {} exp: {}", tag.map, expected); };
        }
    };
};
...
const suite TimingBlock = [] {
    ...
    tag("timing-hardware") / "test_events_and_samples"_test = [] {
        ...
        auto&           timingSrc = testGraph.emplaceBlock<gr::timing::TimingSource>({
            ...
            {"verbose_console", gr::pmt::Value{verbose}}                                                                              //
        });
        auto&           sink      = testGraph.emplaceBlock<TagSink<uint8_t, ProcessFunction::USE_PROCESS_ONE>>({{"name", gr::pmt::Value{"TagSink"}}, {"verbose_console", gr::pmt::Value{verbose}}});
